...
class Solution {
    public int leastInterval(char[] tasks, int n) {
        ...
        Arrays.sort(map);
        // the last line doesn't need any slots
        int maxCnt = map[25] - 1;
        int idleSlotsLeft = maxCnt * n;
        for (int i = 24; i >= 0 && map[i] > 0; i--) {
            idleSlotsLeft -= Math.min(map[i], maxCnt);
        }
        return idleSlotsLeft > 0 ? idleSlotsLeft + tasks.length : tasks.length;
    }
}