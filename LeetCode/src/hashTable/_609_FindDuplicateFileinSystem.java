 ...
 * Note:
 *
 * No order is required for the final output. You may assume the directory name, file name and file content only has
 * letters and digits, and the length of file content is in the range of [1,50]. The number of files given is in the
 * range of [1,20000]. You may assume no files or directories share the same name in the same directory. You may assume
 * each given directory info represents a unique directory. Directory path and file info are separated by a single blank
 * space.
 */
...
class _609_FindDuplicateFileinSystem {
    public List<List<String>> findDuplicate(String[] paths) {
        ...
        for (int i = 0; i < paths.length; i++) {
            ...
            String curPath = curArr[0];
            for (int j = 1; j < curArr.length; j++) {
                String curFile = curArr[j];
                String curContent = curFile.substring(curFile.indexOf("(") + 1, curFile.indexOf(")"));
                String curName = curFile.substring(0, curFile.length() - curContent.length() - 2);
                List<String> curList = map.getOrDefault(curContent, new ArrayList<String>());
                curList.add(curPath + "/" + curName);
                map.put(curContent, curList);
            }
        }
