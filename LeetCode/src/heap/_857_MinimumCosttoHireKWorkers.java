...
class Solution {
    public double mincostToHireWorkers(int[] quality, int[] wage, int K) {
        ...
        double[][] ratioQuality = new double[len][2];
        for (int i = 0; i < len; i++) {
            ratioQuality[i][0] = (double) wage[i] / quality[i];
            ratioQuality[i][1] = quality[i];
        }
        // sort the array in increasing order of ration
        Arrays.sort(ratioQuality, (a, b) -> Double.compare(a[0], b[0]));
        // max heap. 
        Queue<Integer> pq = new PriorityQueue<>((a, b) -> (b - a));
        int qualityCnt = 0;
        for (int i = 0; i < len; i++) {
            pq.offer((int) ratioQuality[i][1]);
            qualityCnt += ratioQuality[i][1];
            // keep K elements in the heap and poll the largest quality
            if (pq.size() > K) {
                qualityCnt = qualityCnt - pq.poll();
            }
            if (pq.size() == K) {
                res = Math.min(res, ratioQuality[i][0] * qualityCnt);
            }
        }
        return res;
    }
