 ...
 * 
 * Example 4: Input: deadends = ["0000"], target = "8888" Output: -1 Note: The
 * length of deadends will be in the range [1, 500]. target will not be in the
 * list deadends. Every string in deadends and the string target will be a
 * string of 4 digits from the 10,000 possibilities '0000' to '9999'.
 * 
 */
    ...
    public int openLock(String[] deadends, String target) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                for (int j = 0; j < 4; j++) {
                    char c = cur.charAt(j);
                    String s1 = cur.substring(0, j) + ((c - '0' + 1) % 10) + cur.substring(j + 1);
                    String s2 = cur.substring(0, j) + ((c - '0' + 9) % 10) + cur.substring(j + 1);
